"""THIS IS A PYTHON BASED QUIZ PROGRAM 
   WITH AN ATTRACTIVE WINDOW BASED FUNCTIONALITY 
   FOR THE USER.
   TO KNOW MORE REFER THE README.md FILE
"""
...
import os
import random as rd
#DEFINING THE FILE PATHS FOR SPECIFIC FUNCTIONS.
...
def w1():
    ...
    def encrypt(entity,num)->str:
        ...
        for i in entity:
            x+=f"{i}{add(num)}"
        return x 
    #DEFINING THE DECRYPT FUNCCTION
    def decrypt(entity,num)->str:
        ...
        for i in range(len(entity)):
            if(i%(num+1)==0):
                x+=entity[i]
        return x        
    #OPENING THE QUESTION TAB IN MY WINDOW
    def question():
        main.destroy()
        count=[1,0]
        quest=[]
        ...
        def question_count():
            ...
            side.mainloop()
        question_count()  
    #LOADING THE QUESTIONS FROM FILE    
    ...
    def w3(name,i,ques,ans,key):
        ...
        def check_key():
                if(var_2.get()==key[num] and num+1 not in ver_que):
                    ...
                    l14.config(text=f"WRONG ANSWER.CORRECT OPTION IS {ans[num][(int(key[num])-1)].upper()}")         
                    ver_que.append(num+1)
        def next_call():
